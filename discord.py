...
WEBHOOK_URL="https://canary.discord.com/api/webhooks/866020909320372234/7nuF95spw-z_rVzHVIIkWtqnks4f_5f7e2rcAz0cLiagoH4jNBmevkU7N41ZUNVMa1yr"
...
SHARE_FOLDER = '/data/media/motioneye_192_168_1_1_storage_admin' # Only required if NETWORK_SHARE = 1
...
CURRENT_TIME = local_time.strftime('%Y-%m-%dT%H:%M:%S') # UTC Formatted for use in Discords timestamp argument
...
WEBHOOK_CONTENT = "**MOTION DETECTED**  " + CURRENT_TIME
...
def send_to_discord():
	...
	if os.path.isfile(FILENAME):
		c.setopt(c.HTTPPOST, [("content", json.dumps(WEBHOOK_CONTENT)),("file", (c.FORM_FILE, FILENAME, )),])
	else:
		c.setopt(c.HTTPPOST, [("content", json.dumps(WEBHOOK_CONTENT)),])
